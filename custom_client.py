...
def encode_request(req: dict) -> str:
    """
    Encode a request dictionary into our custom protocol string with version + numeric codes.

    Format: "version|code|field1|field2|...\n"
    """
    version = PROTOCOL_VERSION  # e.g. "1"
    req_type = req.get("type")
    if req_type not in TYPE_TO_CODE:
        raise ValueError(f"Unknown request type: {req_type}")
    code = TYPE_TO_CODE[req_type]

    fields = []
    if req_type == "create_account":
        # e.g. "1|1|username|password"
        fields = [req.get('username', ''), req.get('password', '')]
    elif req_type == "login":
        fields = [req.get('username', ''), req.get('password', '')]
    elif req_type == "list_accounts":
        fields = [req.get('username', ''), req.get('pattern', '')]
    elif req_type == "send_message":
        fields = [req.get('from', ''), req.get('to', ''), req.get('content', '')]
    elif req_type == "read_new_messages":
        fields = [req.get('username', ''), str(req.get('count', 0))]
    elif req_type == "delete_messages":
        fields = [req.get('username', ''), req.get('message_ids', '')]
    elif req_type == "delete_account":
        fields = [req.get('username', '')]
    elif req_type == "list_messages":
        fields = [req.get('username', '')]
    else:
        raise ValueError(f"Unhandled request type: {req_type}")

    return version + "|" + str(code) + "|" + "|".join(fields) + "\n"

def decode_response(resp_str: str) -> dict:
    """
    Decode a custom protocol response string into a dictionary.

    Success responses: "OK|field1|field2|...|fieldN\n"
    Error responses:   "ERROR|error message\n"
    """
    parts = resp_str.split("|")
    status = parts[0]
    if status == "OK":
        return {"status": "ok", "data": parts[1:]}
    elif status == "ERROR":
        return {"status": "error", "error": parts[1] if len(parts) > 1 else "Unknown error"}
    else:
        raise ValueError(f"Unknown response status: {status}")

def send_request_custom(sock: socket.socket, request: dict) -> dict:
    """
    Send a request using the custom protocol and wait for a response.
    Log the size of the transmitted bytes on the same line.
    """
    req_str = encode_request(request)
    data = req_str.encode("utf-8")

    # Log: "Sending request (size: 37 bytes): {request_dict}"
    logging.info(f"Sending request (size: {len(data)} bytes): {request}")
    ...
    # Log: "Received response (size: 50 bytes): {raw_response}"
    logging.info(f"Received response (size: {len(response_bytes)} bytes): {response_bytes.decode('utf-8').strip()}")
...
class StartFrame(tk.Frame):
    ...
    def create_account(self):
        ...
        try:
            response = send_request_custom(self.controller.sock, req)
